...
You can install the development version of DgeaHeatmap from [GitLab](https://gitlab.ub.uni-bielefeld.de/spittaulab/Dgea_Heatmap_Package) with:
...
if (packageVersion("GeomxTools") < "2.1" &&
    ...
    stop("GeomxTools and Workflow versions do not match. Please use the same
    version. Rhis workflow is meant to be used with most current version of
    packages.If you are using an older version of Bioconductor please reinstall
    GeoMxWorkflows and use vignette(GeoMxWorkflows) instead")
}
...
```{r readInNanostringRawFiles, eval=FALSE, echo=TRUE, results='hide', warning=FALSE}
if (!requireNamespace("GeoMxWorkflows", quietly = TRUE)) {
    message("Das Daten-Paket ist nicht installiert.")
    knitr::opts_chunk$set(eval = FALSE) # Stoppt das Ausführen der Chunks, falls Paket fehlt
}
...
#SampleAnnotationFile <- file.path(datadir, "kidney_AOI_Annotations_all_vignette.xlsx")
...
```{r ReadData, eval=FALSE, echo=TRUE, results='hide', warning=FALSE}
rawDataObject <- suppressWarnings(readNanoStringGeoMxSet(
    dccFiles = subset_dccs,
    pkcFiles = PKCFiles,
...
https://bioconductor.statistik.tu-dortmund.de/packages/3.18/workflows/vignettes/GeoMxWorkflows/inst/doc/GeomxTools_RNA-NGS_Analysis.html (September 21, 2023b). 
Anon Developer Introduction to the NanoStringGeoMxSet. Available from: 
https://www.bioconductor.org/packages/devel/bioc/vignettes/GeomxTools/inst/doc/Developer_Introduction_to_the_NanoStringGeoMxSet.html (September 21, 2023c).
...
```{r examplePreProcess, eval=FALSE, echo=TRUE, results='hide'}
...
file_path <- system.file("extdata", "RawDataExamplePackageNanostring.csv", package = "DgeaHeatmap")
df_Exp <- read.csv(file_path)
copy_df_Expr <- build_matrix(df_Exp, 1)

knitr::kable(head(df_Exp[, seq_len(5)]), "pipe")
path_metadata <- system.file("extdata", "MetaDataPackageNanostring.csv", package = "DgeaHeatmap")
coldata_2 <- read.csv(path_metadata)
coldata_2 <- build_matrix(coldata_2, 1)
``` 
Anon Analyzing GeoMx-NGS RNA Expression Data with GeomxTools. Available from: 
https://bioconductor.statistik.tu-dortmund.de/packages/3.18/workflows/vignettes/GeoMxWorkflows/inst/doc/GeomxTools_RNA-NGS_Analysis.html (September 21, 2023b). 
Anon Developer Introduction to the NanoStringGeoMxSet. Available from: 
...
```{r ColdataDEG, eval=FALSE, echo=TRUE, results='hide', warning=FALSE}
annotation_matrix <- sData(rawDataObject)

annotation_matrix$Samplename <- paste(annotation_matrix$`slide name`, annotation_matrix$class, annotation_matrix$region, annotation_matrix$segment, annotation_matrix$roi, sep = "_")
...
coldata_2 <- data.frame(annotation_matrix$Samplename, annotation_matrix$`slide name`, annotation_matrix$class, annotation_matrix$region, annotation_matrix$segment, annotation_matrix$roi, annotation_matrix$Samplenummern)

# Renaming the columns

names(coldata_2)[names(coldata_2) == "annotation_matrix.Samplename"] <- "Samplename"
names(coldata_2)[names(coldata_2) == "annotation_matrix..slide.name"] <- "slide_name"
...
```{r exampleExchangeColumnNamesWMatchingSampleNames, eval=FALSE, echo=TRUE, results='hide'}
...
knitr::kable(head(copy_df_Expr[, seq_len(5)]), "pipe")
```
...
comparisons <- list(
    "Geometric_Segment_glomerulus_DKD_disease3_vs_PanCK_tubule_DKD_disease4" = c("Geometric_Segment_glomerulus_DKD_disease3", "PanCK_tubule_DKD_disease4"),
    "Geometric_Segment_glomerulus_DKD_disease3_vs_neg_tubule_DKD_disease4" = c("Geometric_Segment_glomerulus_DKD_disease3", "neg_tubule_DKD_disease4"),
    "PanCK_tubule_DKD_disease4_vs_neg_tubule_DKD_disease4" = c("PanCK_tubule_DKD_disease4", "neg_tubule_DKD_disease4")
)
...
result1 <- topTable(fit2, coef = "Geometric_Segment_glomerulus_DKD_disease3 - PanCK_tubule_DKD_disease4", number = Inf, adjust.method = "fdr") %>%
...
```{r ImageVennLimmaupdownreg, echo=FALSE, out.width="100%"}
knitr::include_graphics(
    system.file("extdata", "VennLimmaupdownreg.png", package = "DgeaHeatmap")
)
```
...
```{r ImageVennDESeq2upreg, echo=FALSE, out.width="100%"}
knitr::include_graphics(
    system.file("extdata", "VennDESeq2upreg.png", package = "DgeaHeatmap")
)
```
...
```{r ImageVennDESeq2downreg, echo=FALSE, out.width="100%"}
knitr::include_graphics(
    system.file("extdata", "VennDESeq2downreg.png", package = "DgeaHeatmap")
)
```
...
```{r ImageVennDESeq2sigreg, echo=FALSE, out.width="100%"}
knitr::include_graphics(
    system.file("extdata", "VennDESeq2sigreg.png", package = "DgeaHeatmap")
)
```
...
```{r ImageVennDESeq2updownreg, echo=FALSE, out.width="100%"}
knitr::include_graphics(
    system.file("extdata", "VennDESeq2updownreg.png", package = "DgeaHeatmap")
)
```
...
```{r ImageVennedgeRupdownreg, echo=FALSE, out.width="100%"}
knitr::include_graphics(
    system.file("extdata", "VennedgeRupdownreg.png", package = "DgeaHeatmap")
)
```
